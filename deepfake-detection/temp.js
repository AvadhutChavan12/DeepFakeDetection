...
function sendImageForDetection(imageElement, postElement) {
    ...
    chrome.runtime.sendMessage({ action: "detectDeepfake", imageUrl: imageUrl }, response => {
        ...
        if (response) {
            const confidence = response.confidence ? (response.confidence * 100).toFixed(2) : "N/A";
            postElement.style.border = response.is_deepfake ? "5px solid red" : "5px solid rgb(0, 255, 0)"; // Apply border to the post div
            ...
            label.innerText = response.is_deepfake
